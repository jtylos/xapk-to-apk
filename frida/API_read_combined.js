...
const EXCLUDE_URLS = ["sendbird", "datadog"];
        ...
        BridgeInterceptor.intercept.implementation = function(chain) {
            ...
            if (shouldLogRequest) {
                ...
                if (originalRequestBody) {
                    try {
                        // Use Okio Buffer but extract bytes via reflection to avoid obfuscated method names
                        const OkioBuffer = Java.use("okio.e"); // Obfuscated 'okio.Buffer'
                        const buffer = OkioBuffer.$new();
                        originalRequestBody.writeTo(buffer);
                        
                        // Get byte array by finding the right method dynamically
                        let bodyBytes = null;
                        const bufferClass = buffer.getClass();
                        const methods = bufferClass.getDeclaredMethods();
                        
                        // Look for method that returns byte[] with no parameters
                        for (let i = 0; i < methods.length; i++) {
                            const method = methods[i];
                            if (method.getReturnType().getName() === '[B' && method.getParameterTypes().length === 0) {
                                method.setAccessible(true);
                                bodyBytes = method.invoke(buffer, []);
                                break;
                            }
                        }
                        
                        if (!bodyBytes) {
                            throw new Error("Could not find toByteArray method");
                        }
                        
                        // Cast to proper byte array
                        const ByteArray = Java.use('[B');
                        bodyBytes = Java.cast(bodyBytes, ByteArray);
                        ...
                        if (contentEncoding && contentEncoding.toLowerCase() === 'gzip') {
                            ...
                            const bais = ByteArrayInputStream.$new.overload('[B').call(ByteArrayInputStream, bodyBytes);
                            ...
                            requestBodyString = String.$new.overload('[B', 'java.lang.String').call(String, decompressedBytes, "UTF-8");
                        } else {
                            const String = Java.use('java.lang.String');
                            requestBodyString = String.$new.overload('[B', 'java.lang.String').call(String, bodyBytes, "UTF-8");
                        }
         ...
         // --- DELETE /api/shifts/{id} Modifier (SIMPLIFIED & LESS ERROR-PRONE) ---
            ...
            if (responseBody) {
                try {
                    // Access the buffer without consuming it (peek into internal buffer)
                    ...
                    // Use reflection to find and call the clone method (to preserve original buffer)
                    let clonedBuffer = null;
                    const bufferClass = internalBuffer.getClass();
                    const cloneMethods = bufferClass.getDeclaredMethods();
                    
                    for (let i = 0; i < cloneMethods.length; i++) {
                        const method = cloneMethods[i];
                        // Look for clone method (returns Buffer type, no params)
                        if (method.getReturnType().getName().includes('okio') && 
                            method.getParameterTypes().length === 0 &&
                            !method.getName().includes('read')) {
                            method.setAccessible(true);
                            clonedBuffer = method.invoke(internalBuffer, []);
                            break;
                        }
                    }
                    
                    if (!clonedBuffer) {
                        clonedBuffer = internalBuffer; // Fallback if clone not found
                    }
                    
                    // Use reflection to get byte array from cloned buffer
                    let responseBytes = null;
                    const clonedClass = clonedBuffer.getClass();
                    const byteMethods = clonedClass.getDeclaredMethods();
                    
                    for (let i = 0; i < byteMethods.length; i++) {
                        const method = byteMethods[i];
                        // Look for toByteArray method (returns byte[], no params)
                        if (method.getReturnType().getName() === '[B' && 
                            method.getParameterTypes().length === 0) {
                            method.setAccessible(true);
                            responseBytes = method.invoke(clonedBuffer, []);
                            break;
                        }
                    }
                    
                    if (!responseBytes) {
                        throw new Error("Could not extract bytes from buffer");
                    }
                    
                    // Cast to proper byte array
                    const ByteArray = Java.use('[B');
                    responseBytes = Java.cast(responseBytes, ByteArray);
                    
                    const String = Java.use('java.lang.String');
                    const originalBodyString = String.$new.overload('[B', 'java.lang.String').call(String, responseBytes, "UTF-8");
...
// ================= [END] =================
                    ...
                    const newResponseBody = ResponseBody.create.overload('okhttp3.MediaType', '[B').call(ResponseBody, responseBody.contentType(), responseBytes);
